DROP FUNCTION IF EXISTS public.update_match_details(uuid, integer, text, boolean, boolean, boolean, boolean, jsonb);
...
CREATE OR REPLACE FUNCTION public.update_match_details(
  ...
  p_players jsonb,
  p_conflict_title text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
    ...
    LOOP
      UPDATE public.game_results gr
         SET spice = NULLIF(v_p->>'spice','')::int,
             ...
             combat_strength = NULLIF(v_p->>'combat_strength','')::int,
             garrison_troops = NULLIF(v_p->>'garrison_troops','')::int
       WHERE gr.game_id = p_game_id
...
REVOKE ALL ON FUNCTION public.update_match_details(uuid, integer, text, boolean, boolean, boolean, boolean, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.update_match_details(uuid, integer, text, boolean, boolean, boolean, boolean, jsonb, text) TO authenticated, service_role;